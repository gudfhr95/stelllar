...
export class Migration20221103055255 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "folder" ("id" bigserial primary key, "created_at" timestamptz(0) not null, "name" text not null, "description" text null, "avatar_url" text null, "owner_id" bigint null, "server_id" bigint null, "is_deleted" boolean not null default false, "post_count" int not null default 0, "follower_count" int not null default 0, "is_collaborative" boolean not null default false, "visibility" text check ("visibility" in (\'Public\', \'Friends\', \'Private\', \'Unlisted\')) not null default \'Public\');');

    this.addSql('create table "server_folder" ("server_id" bigint not null, "folder_id" bigint not null, "position" text not null default \'U\', constraint "server_folder_pkey" primary key ("server_id", "folder_id"));');
    this.addSql('alter table "server_folder" add constraint "server_folder_folder_id_unique" unique ("folder_id");');

    this.addSql('create table "folder_post" ("post_id" bigint not null, "folder_id" bigint not null, "added_by_user_id" bigint not null, "added_at" timestamptz(0) not null, constraint "folder_post_pkey" primary key ("post_id", "folder_id", "added_by_user_id"));');
    ...
    this.addSql('create table "channel_user" ("user_id" bigint not null, "channel_id" bigint not null, "last_view_at" timestamptz(0) not null, "mention_count" int not null default 0, constraint "channel_user_pkey" primary key ("user_id", "channel_id"));');
