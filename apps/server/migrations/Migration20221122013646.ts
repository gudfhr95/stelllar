...
export class Migration20221122013646 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "server" ("id" bigserial primary key, "created_at" timestamptz(0) not null, "name" text not null, "display_name" text not null, "description" text null, "owner_id" varchar(255) not null, "category" text check ("category" in (\'Arts\', \'Business\', \'Culture\', \'Discussion\', \'Entertainment\', \'Gaming\', \'Health\', \'Hobbies\', \'Lifestyle\', \'Memes\', \'Meta\', \'News\', \'Politics\', \'Programming\', \'Science\', \'Sports\', \'Technology\', \'Other\')) not null default \'Other\', "user_count" int not null default 0, "avatar_url" text null, "banner_url" text null, "is_deleted" boolean not null default false);');

    this.addSql('create table "server_user" ("user_id" varchar(255) not null, "server_id" bigint not null, "position" text not null default \'U\', "created_at" timestamptz(0) not null, "status" text check ("status" in (\'None\', \'Joined\', \'Banned\')) not null default \'Joined\', constraint "server_user_pkey" primary key ("user_id", "server_id"));');

    this.addSql('create table "post" ("id" bigserial primary key, "created_at" timestamptz(0) not null, "title" text not null, "text" text null, "link_url" text null, "link_metadata" jsonb null, "link_metadatas" jsonb not null, "images" jsonb not null, "author_id" varchar(255) not null, "server_id" bigint not null, "vote_count" int not null default 0, "comment_count" int not null default 0, "updated_at" timestamptz(0) null, "is_deleted" boolean not null default false);');

    this.addSql('create table "post_vote" ("user_id" varchar(255) not null, "post_id" bigint not null, "created_at" timestamptz(0) not null, "type" text check ("type" in (\'Up\', \'None\', \'Down\')) not null default \'None\', constraint "post_vote_pkey" primary key ("user_id", "post_id"));');

    this.addSql('create table "comment" ("id" bigserial primary key, "created_at" timestamptz(0) not null, "author_id" varchar(255) not null, "post_id" bigint not null, "text" text not null, "parent_comment_id" bigint null, "vote_count" int not null default 0, "updated_at" timestamptz(0) null, "is_deleted" boolean not null default false);');

    this.addSql('create table "comment_vote" ("user_id" varchar(255) not null, "comment_id" bigint not null, "created_at" timestamptz(0) not null, "type" text check ("type" in (\'Up\', \'None\', \'Down\')) not null default \'None\', constraint "comment_vote_pkey" primary key ("user_id", "comment_id"));');
