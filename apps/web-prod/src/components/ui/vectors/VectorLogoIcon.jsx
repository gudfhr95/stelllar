export function VectorLogoIcon({ className }) {
  return (
    <svg
      className={className}
      version="1.1"
      viewBox="0 0 6.3938 6.3938"
      xmlns="http://www.w3.org/2000/svg"
    >
      <defs>
        <linearGradient
          id="linearGradient971"
          x1="7.7677"
          x2="14.802"
          y1="5.3857"
          y2="5.3857"
          gradientTransform="rotate(32 322.69 255.38)"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#6875f5" offset="0" />
          <stop stopColor="#f98080" offset="1" />
        </linearGradient>
      </defs>
      <g transform="translate(-188.06 124.29)">
        <path
          d="m192.01-123.61a2.4814 2.526 32 0 0-0.37414 0.0274 1.9313 1.966 32 0 1 0.33022 2.4892 1.9313 1.966 32 0 1-2.3823 0.7953 2.4814 2.526 32 0 0 1.0387 1.3436 2.4814 2.526 32 0 0 3.4427-0.82683 2.4814 2.526 32 0 0-0.76584-3.4572 2.4814 2.526 32 0 0-1.2893-0.37155zm-3.9109 0.15503c-0.20116-0.0233 0.37961 0.32545 2.1063 2.4024 0.31374 0.37744 0.9773 0.0996 1.2604-0.35346 0.28309-0.45304 0.24162-1.1707-0.23513-1.2873l-2.9983-0.73329c-0.0596-0.0146-0.10458-0.0251-0.13332-0.0284z"
          fill="url(#linearGradient971)"
        />
      </g>
    </svg>
  );
}
